package com.epam.jndi2ldap.dao.impl;

import java.util.List;

import com.epam.jndi2ldap.model.UserSettings;

public class UserSetingsDAO {
	public void create(UserSettings userSetings, String baseContext) {
		// super.create(userSetings, baseContext);
	}

	public UserSettings read(String cn, String baseContext) {
		UserSettings userSetings = null;// = super.read(cn, baseContext,
										// javaClassName);
		return userSetings;
	}

	public void update(UserSettings userSetings, String cn, String baseContext) {
		// super.update(userSetings, cn, baseContext);
	}

	public List<UserSettings> readAll(String baseContext, String javaClassName) {
		List<UserSettings> userSetings = null; // = super.readAll(baseContext,
		// javaClassName);
		return userSetings;
	}

}
